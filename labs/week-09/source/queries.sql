...
SELECT r.id, u.name AS requesterName, r.request_type, r.location, r.details, r.status
FROM requests r
...
SELECT DISTINCT u.name, u.department, u.email
FROM users u
JOIN requests r ON u.id = r.requester_id
ORDER BY u.name;

-- ⑧ คำร้อง 3 รายการล่าสุด  (คำใบ้: ORDER BY + LIMIT)
SELECT id, request_type, details, created_at 
FROM requests 
